    ...
    for i in range(3):
        for j in range(3):
            if type(board.items[i][j]) == type(Dummy()):
                ...
                if player == Computer:
                    new_board.items[i][j] = X(None)
                    max_val = minimax(Human, new_board)
                    if max_val > player_computer:
                        player_computer = max_val

                elif player == Human:
                    new_board.items[i][j] = O(None)
                    min_val = minimax(Computer, new_board)
                    if min_val < player_human:
                        player_human = min_val
        ...
        def computerTurn():
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    if type(copy_board.items[i][j]) == type(Dummy()):
                        copy_board.items[i][j] = X(None)
                        score = minimax(Human, copy_board)
                        if score > max_score:
                            max_score = score
                            row = i
                            col = j
